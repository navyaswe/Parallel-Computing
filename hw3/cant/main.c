...
void convert_coo_to_csr(int* row_ind, int* col_ind, double* val, 
                        ...
                        double** csr_vals)
{
    // Memory allocation for csr row ponters, column indices, vals(values)    
    ...
    *csr_vals = (double*)malloc(nnz * sizeof(double));
    for (int i = 0; i <= m; ++i) {
        (*csr_row_ptr)[i] = 0; // row pointers initialization with zero
    }
    // counting no of non-zeroes in every row
    for (int i = 0; i < nnz; ++i) {
        (*csr_row_ptr)[row_ind[i] - 1]++;
    }
    // Get the start index value of each row in an array by summation
    for (int i = 1; i <= m; ++i) {
    ...
    for (int i = 0; i < nnz; ++i) {
        int row = row_ind[i] - 1;
        int index = (*csr_row_ptr)[row]; 
        (*csr_col_ind)[index] = col_ind[i] - 1; // filling array with column indices
        (*csr_vals)[index] = val[i]; // fills array with vals
        (*csr_row_ptr)[row]++; // move to next available position in an array to get access to current row
    }

    // resets row pointers to their starting position
    for (int i = m; i > 0; --i) {
        (*csr_row_ptr)[i] = (*csr_row_ptr)[i - 1];
    }
    (*csr_row_ptr)[0] = 0;
...
void spmv_coo(unsigned int* row_ind, unsigned int* col_ind, double* vals,
              ...
              omp_lock_t* writelock)
{
    double* partial_sums = (double*)malloc(m * sizeof(double)); // memory allocation
	...
	// Adjust row and column to 0-based index
        ...
        if (row >= 0 && row < m && col >= 0 && col < n) {
            #pragma omp atomic
            partial_sums[row] += val * vector_x[col];
        }
    }
          ...
          double* vector_x, double* res)
{
    ...
    for (int i = 0; i < m; ++i) {
        double sum = 0.0;
        // loop to iterate over the non-zero elements in the row
        for (unsigned int j = csr_row_ptr[i]; j < csr_row_ptr[i + 1]; ++j) {
            unsigned int col = csr_col_ind[j];
            double val = csr_vals[j];
	    sum += val * vector_x[col];
        }

        #pragma omp atomic
        res[i] += sum; // update result vector in a safe way based on threads
    }
}
...
void spmv_coo_ser(unsigned int* row_ind, unsigned int* col_ind, double* vals,
                  int m, int n, int nnz, double* vector_x, double* res)
{
    for (unsigned int i = 0; i < nnz; ++i) { // iterates through non-zero elements
        unsigned int row = row_ind[i] - 1;
        unsigned int col = col_ind[i] - 1;
        double val = vals[i];
        if(row < m){ // bounds safety against indices
         if(col < n){

        res[row] += val * vector_x[col];
    }
  }
 }
}
              ...
              double* vector_x, double* res)
{
    for (int i = 0; i < m; ++i) {
        double sum = 0.0;
        // iterates through non-zero elements in the current row accessed from above loop
        for (unsigned int j = csr_row_ptr[i]; j < csr_row_ptr[i + 1]; ++j) {
            unsigned int col = csr_col_ind[j];
            double val = csr_vals[j];
        if(col < n){ // Boundary check for column indices
		sum += val * vector_x[col];
        }
     }
     if(i < m){
        res[i] = sum; // update result for the current row
    }
  }
}
